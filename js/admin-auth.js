...
/* ---------------------------------------------------------------------
   КАК СМЕНИТЬ ПАРОЛЬ
   ---------------------------------------------------------------------
   Обычный способ: admin.html → «Настройки» → «Сменить пароль».
   Панель через api/config.php сама перезапишет константу HASH в этом
   файле на сервере (старая сессия при этом сбрасывается).

   Ручной способ (если API недоступно):
   1. Посчитайте SHA-256 от строки "darles_salt_w79r4l905:НОВЫЙ_ПАРОЛЬ"
      (консоль браузера F12 → Console):

        (async (pwd) => {
          const buf = new TextEncoder().encode("darles_salt_w79r4l905:" + pwd);
          const h = await crypto.subtle.digest("SHA-256", buf);
          console.log("Новый HASH = " +
            Array.from(new Uint8Array(h)).map(b => b.toString(16).padStart(2, "0")).join(""));
        })("НОВЫЙ_ПАРОЛЬ");

   2. В этом файле (на сервере) замените значение константы HASH
      и загрузите файл обратно в папку js/. Войдите с новым паролем.
   --------------------------------------------------------------------- */
